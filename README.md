...
function on_video_update(oldVideo, newVideo) {
  ...
  var username = newVideo.get("userdata.username");
  ...
  if(
    ...
    v1.http.POST({
      "url": url,
      "rawBody": "video_url=" + video_url + "&video_id=" + video_id + "&state=" + video_state + "&embed_url=" + video_embed_url + "&username=" + username
    });
  }
}
```
